...
void update(int node, int b, int e, data t[], int i, int value) {
	if (i < b || e < i ) 
		return;
	...
	if (i == b && i == e) {
		t[node].max = value;
		t[node].sum = value;
		t[node].maxLeft = value;
		t[node].maxRight = value;
		return;
	}

	update(2 * node, b, (b + e) / 2, t, i, value);
	update(2 * node + 1, (b + e) / 2 + 1, e, t, i, value);
